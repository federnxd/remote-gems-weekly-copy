...
import { postImpressions, postReferrals, postHired, postScore, hasEngagement } from '@/lib/post-metrics';
...
export default function IdeaLab() {
  ...
  // Top performers: posts with meaningful engagement (across ALL platforms)
  const topPosts = posts
    .filter(p => p.status === 'published' && hasEngagement(p))
  ...
  const saveMutation = useMutation({
    mutationFn: (idea) => base44.entities.GeneratedPost.create({
      ...
      status: 'draft',
      notes: `platform:linkedin type:job_referral [IdeaLab] structure: ${idea.structure}`,
    }),
    onSuccess: (post, idea) => {
  ...
  const handleGenerate = async () => {
    ...
    const topSummary = topPosts.map(p => ({
      title: p.title,
      strategy: p.strategy,
      impressions: postImpressions(p),
      referrals: postReferrals(p),
      hired: postHired(p),
      notes: p.notes || '',
    }));
    ...
    const strategyPerf = STRATEGY_OPTIONS.map(s => {
      ...
      const avgImpressions = stratPosts.length
        ? Math.round(stratPosts.reduce((sum, p) => sum + postImpressions(p), 0) / stratPosts.length)
        : 0;
  ...
  "ideas": [
    {
      ...
      "content": "Full draft of the LinkedIn post content, ready to use or edit. Write a distinct, human hook (NEVER open with the banned template '📍 [Month] - Remote Opportunities at...'). Include: a genuine hook, a brief personal/credible intro, who should apply, a short roles list, the referral link once, and 3–5 relevant hashtags. Refer to employers only as 'leading AI companies' / 'top AI labs' — never name a specific company."
    }
  ]
}
...
Referral link to always use: https://refer.micro1.ai/referral/jobs?referralCode=eaa2768a-4116-40a1-b897-971506bb359e&utm_source=referral&utm_medium=share&utm_campaign=job_referral`;
